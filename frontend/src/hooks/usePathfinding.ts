...
export function usePathfinding() {
  ...
  const search = useCallback(async (sourceQid: string, targetQid: string, filterCategories?: string[], maxSitelinks?: number | null) => {
    setLoading(true);
    setError(null);
    setResult(null);
    try {
      const data = await findPath(sourceQid, targetQid, 4, filterCategories, maxSitelinks);
      setResult(data);
    } catch (e) {
